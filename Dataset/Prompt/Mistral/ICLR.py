...
def truncate_paper_text(paper_text, prompt_template, max_model_len=32555):
    """
...
def LLM_Generated_Review(user_message, timeout=None):
    ...
    调用 API 并返回生成结果。
    """
    api_url = "http://172.27.33.102:8716/v1/chat/completions"
    model_path = "/mnt/data102_d2/huggingface/models/Mistral-7B-Instruct-v0.3/"
    temperature = 0.9
...
def process_data(data):
    ...
    Here is the paper:
    """
    truncated_text = truncate_paper_text(Paper_Text, prompt1_template)
    prompt1 = f"{prompt1_template}\n\n{truncated_text}\n\nFinally, you only need to output your review texts without any introductory statements."
    ...
    Here is the paper:
    """
    truncated_text = truncate_paper_text(Paper_Text, prompt2_template)
    ...
    Here is the paper:
    """
    truncated_text = truncate_paper_text(Paper_Text, prompt3_template)
    ...
    Here is the paper:
    """
    truncated_text = truncate_paper_text(Paper_Text, prompt4_template)
...
if __name__ == "__main__":
    ...
    Paper_file = "/mnt/data132/taozhen/LLM_Review_Detection/Dataset/Human_Review/ICLR/Papers.json"
    LLM_review_file = "/mnt/data132/taozhen/LLM_Review_Detection/Dataset/LLM_Review/Mistral/ICLR1.json"
